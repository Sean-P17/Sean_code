---
title: "UDA Final Project Sean Patnett"
format: html
---
...
Moving to step 3 Creating the suggestion machine:
So first I am going to get the most common symptoms:
```{python}
...
To ensure I am getting a read on the symptom considering it is one word and I need high probability scores, I am going to combine all the columns(except link) in my orignal dataframe

 ```{python}
 ailment1_df = ailment_df
 ailment1_df['all_info'] = ailment_df['overview'] + ailment_df['symptoms'] + ailment_df['risks'] + ailment_df['cause']
 ailment1_df = ailment1_df.drop(columns=['link_name', 'cause', 'risks', 'overview', 'symptoms'])
 ailment1_df = ailment1_df.reset_index().drop(columns='index')
 ```
...
Now I need to get the user's input and conduct cosine similarity to determine what ailment it most aligns with.
```{python}
#| eval: false
...
Now that I have all the users symptoms(5). I am going to run that through a tfidf vectorizer(the same one).
```{python}
